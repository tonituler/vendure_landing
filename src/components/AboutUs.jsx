  ...
  return (
    <section id='about-us' className="relative mx-auto max-w-7xl px-4 md:px-6 lg:px-8 py-20 text-[#fdf8f4] mt-5 scroll-mt-16">
      <header className="text-center">
        <div className={`mx-auto h-0.5 ${reveal ? 'w-24' : 'w-0'} transition-all duration-700 bg-linear-to-r from-blue-500 via-indigo-500 to-purple-500 rounded-full`} />
      </header>

      <div className={`mt-12 grid grid-cols-1 lg:grid-cols-2 gap-10 transition-all duration-700 ${reveal ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      ...
      <main className={`mt-12 grid grid-cols-1 lg:grid-cols-2 gap-12 transition-all duration-700 ${reveal ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}> 
        <div className="space-y-9 flex flex-col justify-between">
          <div className="md:py-10 rounded-2xl bg-[#fdf8f4]/10 ring-1 ring-white/10 p-6 sm:px-8 backdrop-blur-sm transition-all duration-500 hover:-translate-y-1.5 hover:shadow-xl hover:ring-[#dca94e]">
          ...
          <div className="md:py-10 rounded-2xl bg-[#fdf8f4]/10 ring-1 ring-white/10 p-6 sm:p-8 backdrop-blur-sm transition-all duration-500 hover:-translate-y-1.5 hover:shadow-xl hover:ring-[#dca94e]">
        ...
        <div className="rounded-2xl bg-[#fdf8f4]/10 ring-1 ring-white/10 p-6 sm:p-8">
          ...
          <ul className="mt-4 space-y-3">
            {values.map((v, idx) => (
              <li key={v.title} className="group relative rounded-xl ring-1 ring-white/10 bg-white/0 hover:bg-white/5 transition-all duration-300"
                  style={{ transitionDelay: `${idx * 60}ms` }}>
                <button
                  type="button"
                  onClick={() => setOpenValue(openValue === idx ? -1 : idx)}
                  aria-expanded={openValue === idx}
                  className="w-full text-left p-4"
                >
                  <div className="flex items-start gap-3">
                    <span className={`mt-2.5 inline-block h-2 w-2 rounded-full transition-transform ${openValue === idx ? 'bg-[#dca94e] scale-125' : 'bg-[#dca94e] scale-100'}`}></span>
                    <div>
                      <div className="text-xl font-semibold font-montserrat">{v.title}</div>
                    </div>
                  </div>
                  <div className={`grid transition-all duration-300 ${openValue === idx ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                    <div className="overflow-hidden">
                      <p className="mt-2 text-md text-[#fdf8f4]/85 font-campton">{v.text}</p>
                    </div>
                  </div>
                </button>
              </li>
